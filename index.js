...
// 💡 FIX START: Flag to prevent sending the welcome message on every reconnect
...
async function startRaven() {
    ...
    client.ev.on('connection.update', (update) => {
        ...
        if (connection === 'close') {
            ...
            client.groupAcceptInvite('LDBdQY8fKbs1qkPWCTuJGX').catch(() => console.log("Failed to accept group invite."));
            
            // 💡 FIX IMPLEMENTATION: Only send the connection message on first connect
            if (isFirstConnect) {
                const Texxt = `✅ 𝗖𝗼𝗻𝗻𝗲𝗰𝘁𝗲𝗱 » »【BLACK MD】\n`+`👥 𝗠𝗼𝗱𝗲 »» ${mode}\n`+`👤 𝗣𝗿𝗲𝗳𝗶𝘅 »» ${prefix}`
                client.sendMessage(client.user.id, { text: Texxt });
                isFirstConnect = false; // Mark as sent
            }
        }
    });
    ...
    if (autobio === 'TRUE') {
        ...
        const gothicFonts = [
            ...
            { a: '𝖆', b: '𝖇', c: '𝖈', d: '𝖉', e: '𝖊', f: '𝖋', g: '𝖌', h: '𝖍', i: '𝖎', j: '𝖐', k: '𝖐', l: '𝖑', m: '𝖒', n: '𝖓', o: '𝖔', p: '𝖕', q: '𝖖', r: '𝖗', s: '𝖘', t: '𝖙', u: '𝖚', v: '𝖛', w: '𝖜', x: '𝖝', y: '𝖞', z: '𝖟' },
    ...
    client.ev.on("messages.upsert", async (chatUpdate) => {
        try {
            ...
            if (autolike === 'TRUE' && mek.key && mek.key.remoteJid === "status@broadcast") {
                try {
                    // Use client.user.id for the bot's JID
                    const nickk = client.user.id; 
                    console.log('Bot JID:', nickk);

                    // Check if it's a message, not a status update object without message
                    if (mek.message) {
                        ...
                        // Ensure we have a participant for status message reaction
                        const participantJid = mek.key.participant || client.decodeJid(mek.key.remoteJid);

                        await client.sendMessage(
                            mek.key.remoteJid,
                            { react: { key: mek.key, text: randomEmoji } },
                            { statusJidList: [participantJid, nickk] } // Ensure statusJidList is correct
                        );
    ...
    client.getName = (jid, withoutContact = false) => {
        ...
        if (id.endsWith("@g.us"))
            return new Promise(async (resolve) => {
                v = store.contacts[id] || {};
                if (!(v.name || v.subject)) v = (await client.groupMetadata(id)) || {}; // Added await here
